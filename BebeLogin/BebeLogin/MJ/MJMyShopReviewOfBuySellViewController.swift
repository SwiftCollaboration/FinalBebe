//
//  MJMyShopReviewOfBuySellTableViewController.swift
//  BabyProject
//
//  Created by 김민재 on 2021/08/11.
//

import UIKit

class MJMyShopReviewOfBuySellTableViewController: UIViewController, UITableViewDataSource, UITableViewDelegate{
    
    @IBOutlet weak var segmentedBuySellSelect: UISegmentedControl!
    ...
    //segmented
    @IBAction func segmentedBuySellSelect(_ sender: UISegmentedControl) {
        if sender.selectedSegmentIndex == 0{
            let MJDBItemlist = MJMyShopOfBuyList()
            MJDBItemlist.delegate = self
            MJDBItemlist.downloadItems()
        }else if sender.selectedSegmentIndex == 1{
            let MJDBItemlist = MJMyShopOfSellList()
            MJDBItemlist.delegate = self
            MJDBItemlist.downloadItems()
        }
    }
    ...
    @objc(tableView:cellForRowAtIndexPath:) internal func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cellOfBuySellList", for: indexPath) as! MJMyShopReviewOfBuySellTableViewCell
        ...
        cell.labelScore?.text = "\(item.itemScore!) 점"
...
extension MJMyShopReviewOfBuySellTableViewController:MJSelectOfItemListProtocol{
