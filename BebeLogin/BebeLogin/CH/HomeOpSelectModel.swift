...
protocol HomeOpSelectModelProtocol {
    func itemDownloaded3(items: NSArray)
}

class HomeOpSelectModel: NSObject{
    var delegate: HomeOpSelectModelProtocol!
    var urlPath = Share().url("bebeHomeOp_select.jsp")
    
    func downloadItems(useage:String, category: String){
        let urlAdd = "?useage=\(useage)&category=\(category)"
    ...
    func parseJSON(_ data: Data){
        ...
        DispatchQueue.main.async(execute: {() -> Void in
            self.delegate.itemDownloaded3(items: locations)
        })
